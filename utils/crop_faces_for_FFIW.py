...
def retinaface_crop(model, org_path, save_path, period=1, num_frames=10, args=None):
    ...
    if 'source' in org_path:
        mask_cap_path = org_path.replace('source', 'target_mask').replace('.mp4', '_mask.mp4')
    else:
        mask_cap_path = org_path.replace('target', 'target_mask').replace('.mp4', '_mask.mp4')
    print(mask_cap_path)
    mask_cap = cv2.VideoCapture(mask_cap_path)    
...
if __name__ == '__main__':
    ...
    if args.dataset == 'DFDCP':
        ...
        all_movies = glob(os.path.join(dataset_path, 'source/train/*.mp4'))
        all_movies += glob(os.path.join(dataset_path, 'target/train/*.mp4'))
